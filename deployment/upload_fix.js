...
async function run() {
    ...
    let r = await x('cd /opt/olmec/AI && venv/bin/python3 -c "from olmec_math_engine import OlmecMathEngine; print(\'ENGINE OK\')"');
    ...
    r = await x('cd /opt/olmec/AI && curl -s -X POST -F "file=@nano_banana_collection/banana_og.png" -H "x-api-key: OLMEC_DEV_KEY_99" http://localhost:8000/generate -o /tmp/relief2.glb -w "%{http_code}"');
    console.log('HTTP:', r.stdout);
    r = await x('ls -la /tmp/relief2.glb');
    console.log(r.stdout);
    r = await x('python3 -c "import struct; f=open(\'/tmp/relief2.glb\',\'rb\'); m=struct.unpack(\'<I\',f.read(4))[0]; print(\'VALID\' if m==0x46546C67 else \'BAD\'); f.close()"');
    console.log(r.stdout);

    r = await x('pm2 logs olmec-api --lines 8 --nostream');
    console.log('LOGS:', r.stdout.slice(-600));
